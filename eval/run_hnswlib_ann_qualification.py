#!/usr/bin/env python3
"""AC-79-v1 hnswlib ANN qualification runner (Habit130/squirrel#79).

Writes the freeze before any index build or search.  Legal terminals are
``hnswlib_qualified`` and ``hnswlib_disqualified``.  Does not retune quality
parameters, load Qwen, enable live gamma, copy USearch numbers, or touch
~/Library/Rime.
"""
...
from hnswlib_ann import (  # noqa: E402
    AC164_REPORT_SHA256, ROUTE_ID, Ann79Error,
    assert_ac78_identities, assert_freeze_closed, bge_identity_from_ac164,
    build_freeze, build_report, cell_identity, committed_artifact_dir,
    decide_terminal, file_sha256, load_shortlist_cells, qualify_preset,
...
DEFAULT_WORK = MAIN_REPO / ".local-work" / "ac79-hnswlib-ann" / "work"
DEFAULT_ARTIFACTS = MAIN_REPO / ".local-work" / "ac79-hnswlib-ann" / "artifacts"
...
def _load_ac164_report():
    ...
    if report.get("report_sha256") != AC164_REPORT_SHA256:
        raise Ann79Error("committed AC-164 report SHA-256 drifted")
    return report
...
def run_fixture(work_dir, artifact_dir, committed_dir=None):
    ...
    freeze_path = artifact_dir / "hnswlib_ann_freeze.json"
    ...
    try:
        ...
        preset = HNSWLIB_BUILD_PRESETS[1]
        multiplier = 4
        query_search = 64
    ...
    capacity = {
        ...
        "note": "fixture smoke does not satisfy ANN79-7",
    }
    ...
    report_path = artifact_dir / "hnswlib_ann_report.json"
    markdown_path = artifact_dir / "HNSWLIB_ANN_REPORT.md"
    _write_json(report_path, report)
    markdown_path.write_text(render_markdown(report), encoding="utf-8")
    if committed_dir is not None:
        committed_dir = Path(committed_dir)
        committed_dir.mkdir(parents=True, exist_ok=True)
        for name in ("hnswlib_ann_freeze.json", "hnswlib_ann_report.json",
                     "HNSWLIB_ANN_REPORT.md"):
...
def run_real(work_dir, artifact_dir, snapshot, cache, committed_dir,
    ...
    if digest != PINNED_SNAPSHOT_SHA256:
        raise Ann79Error("isolated snapshot SHA-256 mismatch")
    freeze = build_freeze(
    ...
    from ann import HnswlibIndex, compose_hnswlib_build_params
    try:
        import hnswlib  # noqa: F401
    except ImportError as error:
        raise Ann79Error("hnswlib is required for the real runner") from error
    ...
    suffix_by_key = {}
    try:
        for preset in HNSWLIB_BUILD_PRESETS:
            print("building preset %s" % preset["preset_id"], flush=True)
            build_params = compose_hnswlib_build_params(preset)

            def factory(params=build_params):
                return HnswlibIndex.build([], [], 1024, params)

            grid = qualify_preset(
                replay, provider, finite, factory, (2, 4, 8),
                HNSWLIB_QUERY_SEARCH_VALUES, split="both")
            for (multiplier, query_search), payload in grid.items():
                ...
                prefix_rows.append({
                    ...
                    "overfetch_multiplier": multiplier,
                    "query_search": query_search,
                    "meets_ann79_5": payload["prefix_passing"] > 0,
    ...
    from hnswlib_ann_100k import run_capacity
    from hnswlib_ann_lifecycle import run_lifecycle
    ...
    report = build_report(
        freeze, selection, suffix_cells, capacity, memory, lifecycle,
        True, terminal, code_sha)
    _write_json(artifact_dir / "hnswlib_ann_report.json", report)
    (artifact_dir / "HNSWLIB_ANN_REPORT.md").write_text(
...
def finish_100k(work_dir, artifact_dir, committed_dir, bge_model,
    ...
    artifact_dir = Path(artifact_dir)
    work_dir = Path(work_dir)
    freeze = json.loads((artifact_dir / "hnswlib_ann_freeze.json").read_text(
        encoding="utf-8"))
