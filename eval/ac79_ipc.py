#!/usr/bin/env python3
"""AC-79 daemon IPC helpers (EvidenceService unix socket)."""
...
DAEMON_SCRIPT = os.path.join(_ROOT, "ac79_evidence_daemon.py")
...
def append_commit(facts_root, canonical_input, physical_ms, logical, seq,
    ...
    commit_id = "ac79-commit-%d" % seq
    event_id = "ac79-ev-%d" % seq
    conn.execute(
    ...
    conn.execute(
        ...
        " 'explicit_current',NULL,1,1,'ac79',?,?,?,?,?)",
...
def start_daemon(python, config_path, sock_dir, log_path, status_path,
    ...
    del sock_dir
    short_dir = tempfile.mkdtemp(prefix="ac79s-", dir="/tmp")
    ...
    while time.time() < deadline:
        if os.path.exists(sock_path) and os.path.isfile(status_path):
            try:
                ...
                if status.get("ann_backend") == "hnswlib-hnsw":
    ...
    raise RuntimeError("ac79 daemon did not become ready: %s" % snippet)
...
def stop_daemon(proc, log):
    ...
    if log is not None:
        log.close()
    sock_dir = getattr(proc, "_ac79_sock_dir", None) if proc is not None else None
    if sock_dir:
