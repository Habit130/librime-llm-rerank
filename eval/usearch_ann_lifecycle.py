#!/usr/bin/env python3
"""Isolated real USearch sidecar lifecycle for ANN78-6."""
...
from ann import (  # noqa: E402
    AnnError, USEARCH_BACKEND, USearchIndex, compose_usearch_build_params,
...
from usearch_ann import Ann78Error  # noqa: E402

LIFECYCLE_REPR = "ac78-lifecycle-seed:1024"
...
def _check_restart_load(derived_root, generation_id, expected_ids):
    ...
    if meta.get("backend") != USEARCH_BACKEND:
        raise Ann78Error("restart load used non-usearch backend")
    got = list(loaded.event_ids())
    if got != list(expected_ids):
        raise Ann78Error("restart load event id mismatch")
    probe = loaded.search([1.0] + [0.0] * 1023, 4)
    if not isinstance(probe, list):
        raise Ann78Error("restart load search failed")
    return True
...
def _check_mixed_generation_refuse(derived_root, generation_id):
    try:
        ...
        if error.code == "ann_identity":
            return True
        raise Ann78Error("mixed-generation refuse used %s" % error.code)
    raise Ann78Error("mixed-generation sidecar loaded")
...
def _check_corrupt_rebuild(bundle):
    ...
    with open(ann_path, "wb") as handle:
        handle.write(b"corrupt-usearch-index")
...
def _check_unhealthy_fp32_refuse(bundle):
    ...
    shutil.rmtree(copy_root, ignore_errors=True)
    if not refused:
        raise Ann78Error("unhealthy FP32 did not refuse index rebuild")
    return True


def _check_catch_up_ipc(python, bundle, selection, work_dir):
    from ac78_ipc import (
    ...
    try:
        ...
        if response.get("status") != "ok":
            raise Ann78Error("lifecycle warm query failed: %s" % response)
        ...
        if response.get("error"):
            raise Ann78Error("catch-up returned error %s" % response)
        return True, status
    finally:
...
def run_lifecycle(work_dir, selection, python, qualification_derived=None):
    if not _usearch_available():
        raise Ann78Error("usearch is required for the isolated lifecycle run")
    ...
    isolated = tempfile.mkdtemp(prefix="ac78-life-", dir=work_dir)
    ...
    if qualification_derived:
        ...
        if manifest is not None:
            ...
            result["qualification_sidecar_restore"] = (
                meta.get("backend") == USEARCH_BACKEND
    ...
    result["note"] = (
        "isolated real USearch sidecar lifecycle; EvidenceService IPC catch-up")
