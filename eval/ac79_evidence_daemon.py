#!/usr/bin/env python3
"""AC-79 EvidenceService unix-socket daemon (hnswlib + optional BGE)."""
...
from ann import (  # noqa: E402
    HNSWLIB_BACKEND, load_ann_sidecar, sidecar_dir,
)
...
def _load_index(derived_root, generation_id, query_search):
    ...
    if meta.get("backend") != HNSWLIB_BACKEND:
        raise EvidenceError(
            "ann_identity",
            "ANN sidecar backend %r is not hnswlib-hnsw" % meta.get("backend"))
    return index, meta
    ...
    try:
        ...
        try:
            ...
            rebuild_ann_from_generation(
                ...
                build_params, fingerprint, backend=HNSWLIB_BACKEND)
        finally:
            generation.close()
...
def main():
    ...
    config["retrieval_backend"] = "hnswlib-hnsw"
    ...
    try:
        ...
        print("FAIL: hnswlib sidecar: %s" % error, file=sys.stderr)
        return 2
